...
def generar_datos_simples(cantidad_ejemplos, cantidad_clases):
    FACTOR_ANGULO = 0.79
...
def iniciar(numero_clases, numero_ejemplos, graficar_datos, datos):
    x=[]
    t=[]
    # Generamos datos
    if datos == 0:
        # datos simples
        x, t = generar_datos_simples(numero_ejemplos, numero_clases)
        
    elif datos == 1:
        # datos complejos
        pass
        #x, t = generar_datos_complejos(numero_ejemplos, numero_clases)
...
graf = int(input())
graficar_datos = False if graf == 1 else True

print("0: Datos simples \n 1: Datos complejos")
datos = int(input())

## INICIAR ##
print("Numero de Clases (def 3)")
numero_clases = int(input())

print("Numero de Ejemplos (def 300)")
numero_ejemplos = int(input())

pesos,x,t,loss_values = iniciar(numero_clases, numero_ejemplos, graficar_datos ,datos)

# Generar nuevos datos de entrada
nuevos_datos = np.array([[0.5, 0.8], [0.2, 0.3], [0.7, 0.4]])
...
# Parametro: "c": color (un color distinto para cada clase en t)
plt.scatter(x[:, 0], x[:, 1], c=t)
plt.scatter(nuevos_datos[:, 0], nuevos_datos[:, 1], c=etiquetas_nuevos_datos, marker='x')
plt.show()
...
def generar_datos_complejos(cantidad_ejemplos, cantidad_clases, FACTOR_ANGULO,AMPLITUD_ALEATORIEDAD):
    ...
    for clase in range(cantidad_clases):
        ...
        aleat = yx[indices] * AMPLITUD_ALEATORIEDAD * \
            randomgen.standard_normal(size=n)
        x2 = (yx[indices]+3*x1)**0.2 + aleat
        x[indices] = np.c_[x1, x2]
        t[indices] = clase
